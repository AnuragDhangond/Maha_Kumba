        ...
        <div className="admin-page-content animate-fade-in">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: "40px" }}>
                <div className="dashboard-header-modern">
                    <h1 className="page-title">Global Orders Overview</h1>
                </div>
            </div>
            ...
            <div className="emergency-banner-static">
                <div className="banner-context">
                    ...
                    <div>
                        <h2 className="title-static">Orders Directory</h2>
                        <span className="subtitle-static">Maha Kumbh • Master Order Registry</span>
                    </div>
                </div>
                    ...
                    <thead>
                        <tr>
                            ...
                            <th onClick={() => handleSort('customerName')} style={{ cursor: 'pointer' }}>
                                Customer & Payment {sortConfig.key === 'customerName' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                            </th>
                            <th onClick={() => handleSort('deliveryStatus')} style={{ cursor: 'pointer' }}>
                                Fulfillment Status {sortConfig.key === 'deliveryStatus' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        {orders.length === 0 ? (
                            <tr><td colSpan="3" style={{ textAlign: 'center', padding: '20px', opacity: 0.5 }}>No orders found.</td></tr>
                        ) : (
                            orders.map(o => (
                                <tr key={o.id}>
                                    <td>
                                        <div style={{ fontWeight: '800' }}>#{o.orderNumber}</div>
                                        <div style={{ fontSize: '0.85rem', opacity: 0.6 }}>{new Date(o.createdAt).toLocaleString()}</div>
                                    ...
                                    <td>
                                        <div style={{ fontWeight: '700', color: '#ff7e36' }}>{o.customerName || 'N/A'}</div>
                                        <div style={{ fontSize: '0.85rem', opacity: 0.6 }}>₹{o.totalAmount} • {o.paymentMethod}</div>
                                    </td>
                                    <td>
                                        <span className={`status-pill status-${o.deliveryStatus === 'Delivered' ? 'resolved' : 'pending'}`}>{o.tracking?.currentStatus || o.deliveryStatus || 'N/A'}</span>
                                    </td>
                                </tr>
                            ))
                        )}
